...
A.Q <- function(x) { 
  ...
  ## with cutoffs
  mod.Photo <- ifelse(PARi<=high_light.cutoff, (((IaQY+Amax)-sqrt((IaQY+Amax)^2-(4*convex*IaQY*Amax)))/(2*convex))-Rd[i], 9999)
...
#    box(lwd=2.2)
    ...
    pdf(paste(outdir,sep,file,".pdf",sep=""),height=8,width=10)
    plot(data[,loc1[1]],data[,loc1[2]],pch=21,bg="grey70",cex=3,cex.axis=cexaxis,xlim=c(0,range(data[,loc1[1]])[2]),
         ylim=c(range(data[,loc1[2]])[1]-0.5,range(data[,loc1[2]])[2]+1),cex.lab=cexlab,xlab="PARi",ylab="Photo",main=paste(file))
    box(lwd=2.2)
    ...
    ylim <- range(data[,loc1[2]])
    # Plot all data points
    plot(data[,loc1[1]],data[,loc1[2]], main=paste(file), xlab="PARi", ylab="Photo", cex.lab=cexlab,cex=pt.cex,
         xlim=c(1.1,2000),ylim=c(ylim[1]-1,ylim[2]+3))
    # Plot just the points used in the fit
