...
ok <- require(fitdistrplus) ; if (! ok) {
  install.packages('fitdistrplus', repos="http://cran.rstudio.com/")
} else {
  print("*** Package found: fitdistrplus ***")
}
...
ok <- require(PEcAn.remote) ; if (! ok) {
  library(devtools)
  install_github("PEcAnProject/pecan", subdir="base/remote", ref = "develop") # use development version of PEcAn
} else {
...
refl_data <- read.csv(file=file.path(in_dir,spectra_file),header=T, sep=",")

sample_info_dir <- '~/Data/Dropbox/MANUSCRIPTS/BNL_TEST/Serbin_Arctic_Spectra-Trait/Data/Raw_trait_data/'
sample_info_file <- 'NGEEArctic_BNL_leaf_C_N_LMA_2012-2015.xlsx'
sample_info <- read_excel(path=file.path(sample_info_dir,sample_info_file), sheet = 2)
sample_info[sample_info==-9999] <- NA     

# Chlorophyll data
in_dir <- '/Volumes/TEST/Presentations/Talks/2017/SSerbin_NGEEArctic/Figures/'
chl_data <- 'NGEE_Arctic_chlorophyll_and_carotenoids_Barrow_2013_2015.csv'
chl_data <- read.csv(file=file.path(in_dir,chl_data),header=T, sep=",")
#hist(chl_data$Chl_ab_area_ug_cm2, breaks=30)
#--------------------------------------------------------------------------------------------------#
...
out.dir <- file.path(manuscript_dir,'R_Ouput', 'Inverted_2015_Leaf_Spectra_wPriors/Range_400_700nm')
if (! file.exists(out.dir)) dir.create(out.dir,recursive=TRUE)
#--------------------------------------------------------------------------------------------------#


#--------------------------------------------------------------------------------------------------#
## Fit distribution to Chl data to develop Chl prior
hist(as.vector(chl_data$Chl_ab_area_ug_cm2))
chl_fit <- fitdist(as.vector(chl_data$Chl_ab_area_ug_cm2),distr= "lnorm")
hx <- dlnorm(as.vector(chl_data$Chl_ab_area_ug_cm2), chl_fit$estimate[1], chl_fit$estimate[2])
plot(chl_data$Chl_ab_area_ug_cm2, hx)
#hist(as.vector(chl_data$))
#--------------------------------------------------------------------------------------------------#
...
refl_samp_info <- droplevels(refl_data[,1])
refl_species <- sub("_.*", "", refl_samp_info)
refl_barcode <- gsub("(.*)_(.*)_(.*)_.*", "\\2",  refl_samp_info)

#refl_spec_info2 <- data.frame(Spectra_Name=refl_samp_info, USDA_Species_Code=refl_species, 
#                              Sample_Barcode=refl_barcode)
refl_spec_info2 <- data.frame(Spectra_Name=refl_samp_info, Sample_Barcode=refl_barcode)
sample_info2 <- sample_info[,c(2,3,4,9)]  # just need to keep this
...
prior <- prospect_bt_prior(5, custom_prior=custom_prior)

# Plot title variable
title_var <- "Sample_Barcode"
...
#system.time(for (i in seq_along(1:3) ) {
  ...
  refl_spec_info3 <- droplevels(refl_spec_info2[i,])
  #str(refl_spec_info3)
  output_sample_info <- data.frame(merge(refl_spec_info3, droplevels(sample_info2), by="Sample_Barcode"))
